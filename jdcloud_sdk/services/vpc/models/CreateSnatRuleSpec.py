...
class CreateSnatRuleSpec(object):

    def __init__(self, sourceCidr, description=None, snatElasticIpAddresses=None, snatIpAddresses=None, intelliDstISP=None, ipAffinity=None):
        """
...
如果同时提供，优先使用本参数。
        ...
        :param intelliDstISP: (Optional) 按照运营商智能调度出口IP（ENABLE - 启用； DISABLE - 禁用）注意：公网NAT网关支持该功能，VPC NAT网关暂不支持该功能
        :param ipAffinity: (Optional) 出口IP保持，启用后同一源IP的流量将始终使用相同的出口IP（ENABLE - 启用； DISABLE - 禁用）
        """
