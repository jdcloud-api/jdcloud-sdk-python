...
class ExportLiveStreamBandwidthDataRequest(JDCloudRequest):
    """
    导出带宽数据
- 查询某个时间段内的带宽数据（平均带宽）
- 查询1分钟粒度的数据时，时间跨度不超过7天，其他粒度时时间跨度不超过30天
    ...
    def __init__(self, parameters, header=None, version="v1"):
        super(ExportLiveStreamBandwidthDataRequest, self).__init__(
            '/exportLiveStreamBandwidthData', 'GET', header, version)
        self.parameters = parameters


class ExportLiveStreamBandwidthDataParameters(object):
