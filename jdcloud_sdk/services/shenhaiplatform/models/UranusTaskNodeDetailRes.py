# coding=utf8

# Copyright 2018 JDCLOUD.COM
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#     http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.
#
# NOTE: This class is auto generated by the jdcloud code generator program.


class UranusTaskNodeDetailRes(object):

    def __init__(self, taskName, taskNodeId, taskCode, manager, flowCode, childrenCode=None, parentCode=None, taskDesc=None, taskData=None, nodeName=None, nodeTypeName=None, nodeIcon=None, fileCode=None, jobName=None, isUranus=None, isCurrentFlow=None, parent=None, haveChildren=None, worker=None, taskChangeStatus=None, taskChangeStatusDesc=None, gravityStatus=None, gravityStatusDesc=None, lockStatus=None, lockUser=None):
        """
        :param taskName:  节点名称
        :param childrenCode: (Optional) 子节点code List
        :param parentCode: (Optional) 父节点code List
        :param taskDesc: (Optional) 节点描述
        :param taskNodeId:  任务类型
        :param taskCode:  节点CODE
        :param manager:  负责人
        :param taskData: (Optional) 节点数据 以下前端需要用到的数据
        :param nodeName: (Optional) 节点名称 以下前端需要用到的数据
        :param nodeTypeName: (Optional) 节点类型
        :param nodeIcon: (Optional) 节点图标 以下前端需要用到的数据
        :param flowCode:  工作流Code
        :param fileCode: (Optional) 脚本文件的业务Code
        :param jobName: (Optional) 作业名称
        :param isUranus: (Optional) 是否工作流任务
        :param isCurrentFlow: (Optional) 是否当前工作流任务
        :param parent: (Optional) 给前端用到的
        :param haveChildren: (Optional) 是否有子任务依赖前端用到
        :param worker: (Optional) 协作人
        :param taskChangeStatus: (Optional) 发布变更状态
        :param taskChangeStatusDesc: (Optional) 发布变更状态描述
        :param gravityStatus: (Optional) 作业状态
        :param gravityStatusDesc: (Optional) 作业状态描述
        :param lockStatus: (Optional) 锁定状态 0 无锁 1 锁定 默认 为 1
        :param lockUser: (Optional) 解锁人员
        """

        self.taskName = taskName
        self.childrenCode = childrenCode
        self.parentCode = parentCode
        self.taskDesc = taskDesc
        self.taskNodeId = taskNodeId
        self.taskCode = taskCode
        self.manager = manager
        self.taskData = taskData
        self.nodeName = nodeName
        self.nodeTypeName = nodeTypeName
        self.nodeIcon = nodeIcon
        self.flowCode = flowCode
        self.fileCode = fileCode
        self.jobName = jobName
        self.isUranus = isUranus
        self.isCurrentFlow = isCurrentFlow
        self.parent = parent
        self.haveChildren = haveChildren
        self.worker = worker
        self.taskChangeStatus = taskChangeStatus
        self.taskChangeStatusDesc = taskChangeStatusDesc
        self.gravityStatus = gravityStatus
        self.gravityStatusDesc = gravityStatusDesc
        self.lockStatus = lockStatus
        self.lockUser = lockUser
