...
class SnapshotTemplateSampleConfigInfo(object):

    def __init__(self, startTime=None, intervalTime=None, number=None, format=None, frameType=None, width=None, height=None, fillType=None):
        """
        :param startTime: (Optional) 截图开始时间，单位毫秒，缺省值为 0
        :param intervalTime: (Optional) 截图间隔时间，单位毫秒，缺省值为 0
        :param number: (Optional) 最大截图张数，取值必须大于 0，缺省值为 10
...
若 intervalTime 不为 0, 则从 startTime 开始，依次间隔截图，最多截取 number 张图片；
        ...
        :param width: (Optional) 截图宽度，单位 px，缺省值为 0
        :param height: (Optional) 截图高度，单位 px，缺省值为 0
若 width、height 都不为 0，则截图分辨率为 width * height
若 width、height 某一项为 0，则该项随另一项等比缩放
若 width、height 两项都为 0, 则截图分辨率随源视频

        :param fillType: (Optional) 填充方式。取值范围：
- stretch 伸缩
- black 留黑
- white 留白
- gauss 高斯模糊
