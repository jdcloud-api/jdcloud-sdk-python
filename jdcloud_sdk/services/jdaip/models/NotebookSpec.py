...
- self: 自定义镜像，用户自己保存或上传的镜像
        ...
        :param imageId: (Optional) 镜像ID，用于唯一标识镜像。
...
- 列表顺序决定挂载顺序
        ...
        :param datasets: (Optional) 数据集配置列表，定义Notebook挂载的数据集资源。
...
- 建议挂载到`/mnt/datasets/`路径下
        ...
        :param models: (Optional) 模型配置列表，定义Notebook挂载的模型资源。
...
- 建议挂载到`/mnt/models/`路径下
        ...
        :param permission: (Optional) 工作空间中的资源归属权限，控制资源的可见范围。
...
- 可配置公钥进行免密登录
        ...
        :param userTags: (Optional) 自定义实例标签，用于资源分类和管理。
...
- 示例：{"env": "dev", "team": "ai"}
        ...
        :param resourceGroupId: (Optional) 资源组ID，用于资源分组管理。
