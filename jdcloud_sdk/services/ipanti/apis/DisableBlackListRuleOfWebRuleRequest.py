...
class DisableBlackListRuleOfWebRuleRequest(JDCloudRequest):
    """
    关闭网站类规则的黑名单规则, 批量操作时 webBlackListRuleId 传多个, 以 ',' 分隔, 返回 result.code 为 1 表示操作成功, 为 0 时可能全部失败, 也可能部分失败
    """

    def __init__(self, parameters, header=None, version="v1"):
        super(DisableBlackListRuleOfWebRuleRequest, self).__init__(
            '/regions/{regionId}/instances/{instanceId}/webRules/{webRuleId}/webBlackListRules/{webBlackListRuleId}:disable', 'POST', header, version)
        self.parameters = parameters


class DisableBlackListRuleOfWebRuleParameters(object):
