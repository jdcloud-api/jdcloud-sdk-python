...
- 可调用接口（describeOS）获取分布式云物理服务器支持的操作系统列表
 [MFA enabled]
    ...
    def __init__(self, parameters, header=None, version="v1"):
        super(ReinstallInstanceRequest, self).__init__(
            '/regions/{regionId}/instances/{instanceId}:reinstallInstance', 'PUT', header, version)
        self.parameters = parameters


class ReinstallInstanceParameters(object):

    def __init__(self, regionId, instanceId, instanceSpec):
        """
        :param regionId: 地域ID，可调用接口（describeEdCPSRegions）获取分布式云物理服务器支持的地域
        :param instanceId: 分布式云物理服务器ID
        :param instanceSpec: 分布式云物理服务器配置
        """
