...
class AsyncAudioScanRequest(JDCloudRequest):
    """
    提交音频异步检测任务
    """

    def __init__(self, parameters, header=None, version="v1"):
        super(AsyncAudioScanRequest, self).__init__(
            '/audio:asyncscan', 'POST', header, version)
        self.parameters = parameters


class AsyncAudioScanParameters(object):
    ...
    def setTasks(self, tasks):
        """
        :param tasks: (Optional) 检测任务列表，包含一个或多个元素。每个元素是个结构体，最多可添加10个元素，每个元素的具体结构描述见audioTask。
        """
        self.tasks = tasks
