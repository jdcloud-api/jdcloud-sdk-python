...
class WorkspaceRelatedQueue(object):
    ...
    def __init__(self, queueId=None, allModuleSupported=None, queueModules=None, allMemberSupported=None, queueMemberPins=None):
        ...
        :param allModuleSupported: (Optional) true支持所有任务模块,默认true。 注意：安全队列仅支持训练任务，不能设置为true
        :param queueModules: (Optional) 支持使用任务模块，allModuleSupported=false生效。 注意：安全队列仅支持训练任务
可选值：
- notebook:Notebook
- training:训练任务
- inference:在线服务，
