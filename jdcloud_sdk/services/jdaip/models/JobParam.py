...
- 挂载测试数据集
        ...
        :param models: (Optional) 模型配置列表，用于挂载预训练模型或检查点文件。
...
- 挂载基础模型进行微调
        ...
        :param roleResource: (Optional) 角色资源配置信息，用于配置不同角色的节点资源。
...
**示例：**
  {"ray": {"env_vars": {"NCCL_DEBUG": "INFO", "NCCL_IB_DISABLE": "0", "NCCL_PROTO": "simple"}}}
        ...
        :param restartPolicy: (Optional) 重启策略配置，定义任务异常时的自动重启行为。
...
- 快速实验任务：可不启用，快速发现问题
        ...
        :param healthCheckPolicy: (Optional) 算力健康检测策略，用于检测GPU等算力资源是否正常。
...
**默认值：** `public`
        ...
        :param nodeAffinities: (Optional) 节点亲和性配置，用于将任务调度到特定的计算节点。支持配置多个亲和性规则，每个元素之间是"或"的关系。
...
**注意：** 配置不当可能导致任务无法调度
        ...
        :param codes: (Optional) 代码仓库配置列表，用于从代码仓库拉取训练代码。
...
- 资源分类管理
        ...
        :param resourceGroupId: (Optional) 资源组ID，用于将任务归属到特定的资源组进行管理。
