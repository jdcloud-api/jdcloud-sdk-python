...
class CreateSnapshotTemplateRequestInfo(object):

    def __init__(self, title, startTime=None, frameType=None, format=None, number=None, interval=None, width=None, height=None, fillType=None, spriteConfig=None, configData=None):
        """
...
缺省值为 any
        ...
        :param format: (Optional) 截图格式。取值范围：
  jpg - 对应的截图或雪碧图输出文件扩展名为 jpg
  png - 对应的截图或雪碧图输出文件扩展名为 png
缺省值为 jpg

        :param number: (Optional) 截图数量。取值范围：[1, 100000]
缺省值为 10

        :param interval: (Optional) 截图时间间隔。单位为毫秒
...
缺省值为 black
        ...
        :param spriteConfig: (Optional) 雪碧图配置。若此字段有值，则生成雪碧图。
