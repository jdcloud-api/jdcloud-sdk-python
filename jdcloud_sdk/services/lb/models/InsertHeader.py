...
class InsertHeader(object):

    def __init__(self, valueType=None, key=None, value=None):
        """
        :param valueType: (Optional) http header的value类型：1.系统值类型；2.自定义值类型
        :param key: (Optional) 新增http header的key，必填，最长40个字符，不区分大小写，支持大小写字母、数字、下划线（_）和短划线（-），不允许使用系统header+部分http标准header，同一个key在插入/删除中只能出现一次，具体不允许的key如下：
...
2.部分http标准header：connection、upgrade、content-length、transfer-encoding、keep-alive、te、host、cookie
        ...
        :param value: (Optional) 新增http header的value，必填，取值包括两种：
