...
class DescribeQueryOption(object):

    def __init__(self, aggrType=None, dimensions=None, downSampleType=None, metric=None, metricName=None):
        """
        :param aggrType: (Optional) 聚合方式，默认等于downSampleType(若downSampleType为last，AggrType取max)或avg，可选值参考:sum、avg、min、max
        :param dimensions: (Optional) 监控指标数据的维度信息,根据tags来指定查询的监控数据维度，至少指定一个查询维度
