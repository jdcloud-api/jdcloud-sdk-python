# coding=utf8

# Copyright 2018 JDCLOUD.COM
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#     http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.
#
# NOTE: This class is auto generated by the jdcloud code generator program.

from jdcloud_sdk.core.jdcloudrequest import JDCloudRequest


class DescribeRedisServerLogRequest(JDCloudRequest):
    """
    查询redis服务端日志
    """

    def __init__(self, parameters, header=None, version="v1"):
        super(DescribeRedisServerLogRequest, self).__init__(
            '/regions/{regionId}/cacheInstance/{cacheInstanceId}/redisServerLog', 'GET', header, version)
        self.parameters = parameters


class DescribeRedisServerLogParameters(object):

    def __init__(self,regionId, cacheInstanceId, ):
        """
        :param regionId: 缓存Redis实例所在区域的Region ID。目前有华北-北京、华南-广州、华东-上海三个区域，Region ID分别为cn-north-1、cn-south-1、cn-east-2
        :param cacheInstanceId: 缓存Redis实例ID，是访问实例的唯一标识
        """

        self.regionId = regionId
        self.cacheInstanceId = cacheInstanceId
        self.podName = None
        self.logFilter = None
        self.startTime = None
        self.endTime = None
        self.pageNumber = None
        self.pageSize = None

    def setPodName(self, podName):
        """
        :param podName: (Optional) pod 名称，多个以逗号分隔
        """
        self.podName = podName

    def setLogFilter(self, logFilter):
        """
        :param logFilter: (Optional) 查询筛选信息
        """
        self.logFilter = logFilter

    def setStartTime(self, startTime):
        """
        :param startTime: (Optional) 查询开始时间
        """
        self.startTime = startTime

    def setEndTime(self, endTime):
        """
        :param endTime: (Optional) 查询结束时间
        """
        self.endTime = endTime

    def setPageNumber(self, pageNumber):
        """
        :param pageNumber: (Optional) 页码：取值范围[1,∞)，默认为1
        """
        self.pageNumber = pageNumber

    def setPageSize(self, pageSize):
        """
        :param pageSize: (Optional) 分页大小：取值范围[10, 100]，默认为10
        """
        self.pageSize = pageSize

