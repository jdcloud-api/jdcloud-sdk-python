...
class VideoCallbackV2Request(JDCloudRequest):
    """
    视频检测结果获取接口，获取离线处理的数据后，下次调用，不会再次返回之前获取过的离线数据。小于10s/20次，请求频率过快服务器会拒绝处理
    """

    def __init__(self, parameters, header=None, version="v1"):
        super(VideoCallbackV2Request, self).__init__(
            '/video:callbackv2', 'POST', header, version)
        self.parameters = parameters


class VideoCallbackV2Parameters(object):
